...
**你必须避免单一情绪带陷阱**：如果一批选题全是"关系里的隐性伤害"（讨好型/边界感/为你好的绑架/情绪吸血鬼等"丧、清醒、止损"类），内容高度同质化，算法和用户都会疲劳。每批必须主动覆盖至少 4 种不同情绪：
- 愤怒共鸣（被PUA/被不公对待的"凭什么"）
- 搞笑自嘲（社死/恋爱翻车的"笑死我也是"）
- 怀旧刺痛（前任/青春/回不去的"那时候不懂"）
- 疗愈陪伴（自我和解/独处/被理解的"被懂了"）
- 实用洞察（识人/沟通/避雷的"早知道就好了"）

**每批至少 2 个选题必须是用户会主动搜索的问句**（如"30岁不结婚真的可怕吗""怎么判断男朋友爱不爱你"），带搜索量的问句标题能带来长尾流量，是新号起量的关键。

**系列归属（每个选题必须归入一个固定系列，让读者有"追更"理由，提升关注转化）**：
- "互联网打工人的深夜"（职场+情感/人际的隐喻观察）
- "清醒日记"（自我成长/止损/边界）
- "独居生存指南"（独居/生活态度/安全感）
- "不敢说出口的话"（亲密关系/沟通/暗恋分手）
- "原来如此"（认知反转/概念解读/反常识）"""
...
- 要像真实的人在说话，不是营销号
- 关键词要贴近小红书用户的搜索习惯
- **情绪光谱均衡（硬性）**：10个选题必须覆盖至少4种情绪（愤怒共鸣/搞笑自嘲/怀旧刺痛/疗愈陪伴/实用洞察），严禁全部是"关系隐性伤害"类（讨好型/边界感/为你好/情绪吸血鬼等）。同质化=流量死局
- **搜索型选题配额（硬性）**：至少2个选题是用户会主动搜索的问句标题（带问号、针对具体搜索意图），用于获取长尾搜索流量
  ...
  "topics": [
    {
      ...
      "series": "互联网打工人的深夜/清醒日记/独居生存指南/不敢说出口的话/原来如此"
    }
  ]
}
```"""
...
def generate_topics_batch(batch_index: int, existing_topics: list[str], trending_keywords: list[str] | None = None) -> list[dict]:
    ...
    for i, topic in enumerate(topics):
        ...
        required_fields = ["topic", "title_formula", "target_interaction", "angle", "keywords", "pillar", "story_prototype", "controversy_anchor", "series"]
